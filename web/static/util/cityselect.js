...
Vcity._m = {
        ...
        if (typeof arg == 'string') {
            switch (arg.charAt(0)) {
                ...
                case '.':
                    ...
                    tagAll = Vcity._m.$('*', context);//根据标签名进行选择
                    n = tagAll.length;
                    for (i = 0; i < n; i++) {
    ...
    getPos:function (node) {
        var scrollx = document.documentElement.scrollLeft || document.body.scrollLeft,//横向滚动
                scrollt = document.documentElement.scrollTop || document.body.scrollTop;//竖直滚动
        //获得滚动的距离
        var pos = node.getBoundingClientRect();//获得相对于视窗的位置集合
...
Vcity.regEx = /^([\u4E00-\u9FA5\uf900-\ufa2d]+)\|(\w+)\|(\w)\w*$/i;//筛选中文或者拼音
Vcity.regExChiese = /([\u4E00-\u9FA5\uf900-\ufa2d]+)/;//筛选中文
...
(function () {
    ...
    if (!Vcity.oCity) {
        ...
        for (var i = 0, n = citys.length; i < n; i++) {
            match = regEx.exec(citys[i]);//exec返回的结果数组的0位置就是与其相匹配的文本
            letter = match[3].toUpperCase();//将匹配的第三个子元素 即简拼全部转换为大写
            if (reg2.test(letter)) {
                if (!Vcity.oCity.AB[letter]) Vcity.oCity.AB[letter] = [];
                Vcity.oCity.AB[letter].push(match[1]);//将中文城市名放入数组
            ...
            //注意 0 位置放的是匹配的文本 1 位置放的是第一个子表达式匹配的文本 2 3 以此类推
            //将前几个热门城市加入热门城市数组
        }
    }
})();
...
Vcity.CitySelector.prototype = {
    ...
    initialize :function (options) {
        ...
        this.input = Vcity._m.$('#'+ input);//通过id选择器选择元素
        this.target= Vcity._m.$('#'+ target);
        this.inputEvent();//调用input的输入监听
    },
    ...
    createWarp:function(){
        var inputPos = Vcity._m.getPos(this.input);//得到当前input的位置用于定位弹出框的位置
        var div = this.rootDiv = document.createElement('div');//创建弹出框的根节点
        var that = this;//保存this this指向citySelector
        // 设置DIV阻止冒泡
        Vcity._m.on(this.rootDiv,'mouseover',function(event){
            Vcity._m.stopPropagation(event);//若在当前citySelector上方的时候给其
            Vcity._m.addClass('selectCityHover',that.input);
           if(!!that.cityBox && Vcity._m.hasClass('hide',that.cityBox)){


                Vcity._m.removeClass('hide',that.cityBox);//显示城市选择框 用于决定是显示ui 还是 城市选择框
        ...
        Vcity._m.on(this.rootDiv,'mouseout',function(event){
            event = event || window.event;
            if(!!that.cityBox && !Vcity._m.hasClass('hide',that.cityBox)){
                Vcity._m.removeClass('selectCityHover',that.input);
        ...
        // 设置点击文档隐藏弹出的城市选择框
        // Vcity._m.on(document, 'mouseover', function (event) {
        //     event = Vcity._m.getEvent(event);//得到点击的事件对象
        //     var target = Vcity._m.getTarget(event);//得到点击的触发元素
        //     if(target == that.input) return false;//如果点击对象是当前的input对象 则返回 什么都不做
        //     //console.log(target.className);
        //     if (that.cityBox)Vcity._m.addClass('hide', that.cityBox);//将弹出的显示框隐藏
        //     if(that.myIframe)Vcity._m.addClass('hide',that.myIframe);
        // });
        div.className = 'citySelector';//给根div添加样式 并为其定位
        div.style.position = 'fixed';
        div.style.left = inputPos.left+'px';
        div.style.top = inputPos.bottom-1+'px';
        div.style.zIndex = 501;

        //使显示框浮在最上方 IE6的 zindex失效 需要为其添加iframe
        // 判断是否IE6，如果是IE6需要添加iframe才能遮住SELECT框
        var isIe = (document.all) ? true : false;//判断是否为ie浏览器
        var isIE6 = this.isIE6 = isIe && !window.XMLHttpRequest;//判断是否为ie6
        if(isIE6){
        ...
        //若为ie6则为其添加一个iframe的遮罩
        var childdiv = this.cityBox = document.createElement('div');//创建子盒子
        childdiv.className = 'cityBox';//添加样式 和 属性
        childdiv.id = 'cityBox';
        childdiv.innerHTML = Vcity._template.join('');//导入城市html模板
        var hotCity = this.hotCity =  document.createElement('div');//创建热门城市
        hotCity.className = 'hotCity';
        childdiv.appendChild(hotCity);
        div.appendChild(childdiv);
        this.createHotCity();//调用createHotCity方法 创建城市内容
    },
    ...
    createHotCity:function(){
        var odiv,odl,odt,odd,odda=[],str,key,ckey,sortKey,regEx = Vcity.regEx,
                oCity = Vcity.oCity;
        for(key in oCity){//取出存放的所有城市 为其每一个分组创建一个div做为tab选项卡的容器
            odiv = this[key] = document.createElement('div');//赋值给当前的citySelector对象
            //此处创建了this.hot this.ab等属性
            // 先设置全部隐藏hide
            odiv.className = key + ' ' + 'cityTab hide';//设置样式 同时添加当前的分组 便于进行选择
            sortKey=[];//用于分组内部的排序
            for(ckey in oCity[key]){//将分组内部的简写拼音取出 并进行排序
                sortKey.push(ckey);
                // ckey按照ABCDEDG顺序排序
                sortKey.sort();
            }
            for(var j=0,k = sortKey.length;j<k;j++){
                odl = document.createElement('dl');//分组内的每一个小组创建dl表格
                odt = document.createElement('dt');//
                odd = document.createElement('dd');
                odt.innerHTML = sortKey[j] == 'hot'?'&nbsp;':sortKey[j];
                odda = [];//创建每一个单独的城市 并保存到odda中
                ...
                odl.appendChild(odt);
                odl.appendChild(odd);
                odiv.appendChild(odl);//添加到odiv中 此时odiv均隐藏
            }
        ...
        document.body.appendChild(this.rootDiv);//将其放到根节点下
        /* IE6 */
        this.changeIframe();//切换iframe的状态一次

        this.tabChange();//绑定tab切换事件
        this.linkEvent();
    },
    ...
    tabChange:function(){
        var lis = Vcity._m.$('li',this.cityBox);//得到上方所有的分组
        var divs = Vcity._m.$('div',this.hotCity);//得到下方所有的div
        var that = this;//保留this
        for(var i=0,n=lis.length;i<n;i++){
            lis[i].index = i;//为添加index属性 让其记住自己的编号
            lis[i].onclick = function(){//绑定点击事件 进行tab切换动作的入口
                for(var j=0;j<n;j++){
                    Vcity._m.removeClass('on',lis[j]);//移除所有li的on样式
                    Vcity._m.addClass('hide',divs[j]);//为所有div添加hide样式
                }
                Vcity._m.addClass('on',this);//给当前的li添加on样式
                Vcity._m.removeClass('hide',divs[this.index]);//为当前li对应的div移除hide样式
                /* IE6 改变TAB的时候 改变Iframe 大小*/
                that.changeIframe();//切换Iframe()一次
            };
        }
    },
    ...
    linkEvent:function(){
        var links = Vcity._m.$('a',this.hotCity);//获得所有的城市标签 并为其绑定点击事件 当点击时其内容进入input中
        var that = this;//保留this为当前对象
        for(var i=0,n=links.length;i<n;i++){//遍历所有的a 并为其绑定点击事件
            links[i].onclick = function(e){
                e.preventDefault();
                that.target.innerHTML = this.innerHTML;//此处的this指向点击的a标签
                that.target.title=this.innerHTML;
                Vcity._m.addClass('hide',that.cityBox);//隐藏城市选择框
                /* 点击城市名的时候隐藏myIframe */
                Vcity._m.addClass('hide',that.myIframe);//隐藏iframe 用于兼容ie6
                Vcity._m.removeClass('selectCityHover',that.input);
            };
        }
    },
    ...
    inputEvent:function(){
        var that = this;//保留this的当前对象

        Vcity._m.on(this.input,'mouseover',function(event){
            event = event || window.event;
            Vcity._m.addClass('selectCityHover',that.input);
            if(!that.cityBox){//若城市选择框不存在就掉用createWrap()方法创建城市选择框
                that.createWarp();
        ...
        Vcity._m.on(this.input,'mouseout',function(event){
            event = event || window.event;
           if(!!that.cityBox && !Vcity._m.hasClass('hide',that.cityBox)){
                    Vcity._m.removeClass('selectCityHover',that.input);
                    Vcity._m.addClass('hide',that.cityBox);
        ...
        // });
        Vcity._m.on(this.input,'blur',function(){//当前输入框失去焦点时
            // if(that.input.value == '') that.input.value = '城市名';
            
            var value = Vcity._m.trim(that.input.value);//取出输入框中的输入的内容
            if(value != ''){//若输入的内容不为空
                var reg = new RegExp('^' + value + '|\\|' + value, 'gi');//生成一个正则表达式
                var flag=0;
